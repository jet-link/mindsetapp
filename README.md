...
PATCH  /api/v1/me/language/            # язык интерфейса (en/ru/uz)
...
Поддерживаются три языка: **English (по умолчанию и fallback)**, **Russian**, **Uzbek**
(коды `en`/`ru`/`uz`). Переключение языка происходит мгновенно, без перезагрузки страницы.

Как это устроено:

- **Frontend:** `i18next` + `react-i18next`. Переводы лежат в `frontend/locales/<lang>/<namespace>.json`,
  где namespace = функциональный модуль (`common`, `auth`, `feed`, `profile`, `search`,
  `notifications`, `settings`, `messages`, `moderation`, `errors`, `dates`). Английский
  бандлится статически, `ru`/`uz` подгружаются лениво при первом переключении.
- **Хранение языка:** гость — в `localStorage` (`mindset-locale`); авторизованный — в профиле
  (`User.language`) с синхронизацией через `PATCH /api/v1/me/language/`. При входе server wins,
  если язык в профиле отличается от дефолтного.
- **Backend:** все запросы шлют `Accept-Language`. `LanguageMiddleware` кладёт язык в `request.language`
  и contextvar; ошибки/системные строки переводятся через `t(key, lang)` с fallback на `en`
  (переводы в `backend/apps/core/i18n/locales/<lang>/`).
- **Даты и числа** форматируются на клиенте через `Intl` (`formatRelativeTime`, `formatCompactNumber`,
  `formatDateTime` из `frontend/lib/i18n`), чтобы смена языка применялась без перезапроса данных.

Правила для новых строк:

- Никакого хардкода UI-текста — каждая видимая строка идёт через `t('namespace:key')`.
- Сначала добавляем ключ в `en`, затем в `ru` и `uz` (полный набор во всех трёх языках).
- Ключи всегда фиксированные; **никогда** `t(userInput)`.
- Пользовательский контент (посты, ответы, bio, имена) **не переводится**.
- Для множественного числа используем суффиксы i18next (`key_one`/`key_few`/`key_many`/`key_other`),
  переменные — через `{{name}}`.
- Чек-лист для PR: в JSX нет новых литеральных строк (placeholder/aria-label/текст без `t(`).
